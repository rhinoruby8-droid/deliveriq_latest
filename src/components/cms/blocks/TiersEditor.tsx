...
export function TiersEditor({ value, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4 p-5 bg-[#1A1D24]/60 border border-[#2C2F38] rounded-sm">
      <p className="text-[10px] font-bold text-[#C79A4E] uppercase tracking-widest">Sponsorship Tiers</p>
      {safe.map((tier, i) => (
        <div key={i} className="flex flex-col gap-3 p-4 border border-[#2C2F38] rounded-sm relative">
          <button onClick={() => removeTier(i)} className="absolute top-3 right-3 p-1 text-[#8A8D96] hover:text-red-400 transition-colors"><Trash2 size={12} /></button>
          ...
          <div className="flex flex-col gap-2">
            ...
            {tier.benefits.map((b, j) => (
              <div key={j} className="flex items-center gap-2">
                ...
                <button onClick={() => removeBenefit(i, j)} className="shrink-0 p-1.5 text-[#8A8D96] hover:text-red-400 transition-colors"><Trash2 size={11} /></button>
              </div>
            ))}
            <button onClick={() => addBenefit(i)} className="flex items-center gap-1 text-xs text-[#C79A4E] hover:text-[#F0EDE8] transition-colors"><Plus size={11} /> Add benefit</button>
          </div>
        </div>
      ))}
      <button onClick={addTier} className="flex items-center gap-1.5 text-xs font-semibold text-[#C79A4E] hover:text-[#F0EDE8] transition-colors"><Plus size={13} /> Add Tier</button>
    </div>
  );
}
